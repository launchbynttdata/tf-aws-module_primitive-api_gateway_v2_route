# tf-aws-module_primitive-api_gateway_v2_route
...
Provides an API Gateway Route resource.
...
| <a name="input_route_key"></a> [route\_key](#input\_route\_key) | Route key for the route. For HTTP APIs, the route key can be either `$default`, or a combination of an HTTP method and resource path, for example, `GET /pets`. | `string` | `"$default"` | no |
| <a name="input_target"></a> [target](#input\_target) | Target for the route, of the form `integrations/IntegrationID`, where `IntegrationID` is the identifier of an `aws_apigatewayv2_integration` resource. | `string` | `null` | no |
